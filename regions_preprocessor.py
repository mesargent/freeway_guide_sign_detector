class RegionsPreprocessor(Preprocessor):
	...
	def process(self, image):
		lower = np.array(config["preprocessing_params"]["colors_lower"], dtype="uint8")
    	upper = np.array(config["preprocessing_params"]["colors_higher"], dtype="uint8")
    	mask = cv2.inRange(image, self, upper)
